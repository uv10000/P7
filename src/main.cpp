        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			*/
			...
			// if previous size ist almost empty use the car as starting reference
			if(prev_size <2) {
			  ...
			  ptsy.push_back(car_y);
				...
				//redefine referenc state as previous path end point
				...
				// Use tow points that make the path tange3nt to the previous path's end point
			...
			//in Frenet add evenly 30 m spaced points ahed of the datarting reference. 
			vector<double> next_wp0 = getXY(car_s+30,(2+4*lane), map_waypoints_s, map_waypoints_x, map_waypoints_y);  // 1 was lane
			...
			for (int i=0;i<ptsx.size();i++) {
				// shtift car reference angle to 0 degrees.
			...
			*/
			...
			//calculate how to break up spline points so that w travel at aour desired reference velocity
			...
			//fill up the rest of our path planner after filling it with previous points, her we will always output
